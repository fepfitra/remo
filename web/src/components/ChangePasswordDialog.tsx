...
const ChangePasswordDialog: React.FC<Props> = ({ destroy }: Props) => {
  ...
  const handleSaveBtnClick = async () => {
    if (oldPassword === "" || newPassword === "" || newPasswordAgain === "") {
      toastHelper.error("Password cannot be empty");
      return;
    }
    ...
    try {
      ...
      if (!isValid) {
        toastHelper.error("Old password incorrect");
        setOldPassword("");
        return;
      }

      await userService.updatePassword(newPassword);
      toastHelper.info("Password changed successfully!");
      handleCloseBtnClick();
    } catch (error: any) {
  ...
  return (
    <>
      ...
      <div className="dialog-content-container">
        ...
        <label className="form-label input-form-label">
          <span className={"normal-text " + (newPassword === "" ? "" : "not-null")}>New Password</span>
          <input type="password" value={newPassword} onChange={handleNewPasswordChanged} />
        </label>
        <label className="form-label input-form-label">
          <span className={"normal-text " + (newPasswordAgain === "" ? "" : "not-null")}>Repeat New Password</span>
