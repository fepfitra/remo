...
const CreateQueryDialog: React.FC<Props> = (props: Props) => {
  ...
  const handleSaveBtnClick = async () => {
    if (!title) {
      toastHelper.error("Title cannot be empty!");
      return;
    }
  ...
  const handleAddFilterBenClick = () => {
    if (filters.length > 0) {
      ...
      if (lastFilter.value.value === "") {
        toastHelper.info("Please complete the previous filter first");
        return;
      }
    }
      ...
      <div className="dialog-footer-container">
        ...
        <div className="btns-container">
          <span className={`tip-text ${filters.length === 0 && "hidden"}`}>
            Memos matching condition: <strong>{shownMemoLength}</strong>
          </span>
